...
import { resetPortalPassword } from "@/lib/portal-api";

function PortalSetPasswordForm() {
  ...
  async function handleSubmit(e) {
    ...
    if (!token) {
      setError("This invite link is missing its token. Please ask your engagement team to resend it.");
      return;
    }
    ...
    try {
      setLoading(true);
      await resetPortalPassword(token, password);
      setSuccess(true);
      ...
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Choose a password to activate your client portal account.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!token ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                No invite token found in this link. Please use the link from your invite
                email, or contact your engagement team for a new one.
              </div>
            ) : null}
